function job_burst_pattern_original_backup(arrayID)
    ...
    for iBurst = 1: size(CC.PixelIdxList,2)
      ...
      % 
        for iTime = burstTimeStart:burstTimeEnd  
            ...
            instantPeakAmp(iTime,count,:) = max(max(currentBurst(:,:,iTime))) ;
            instantTotalPower(iTime,count,:) = sum(sum(currentBurst(:,:,iTime).^2) ) ;
...
saveFileName = ['FullBurst90%',dataFileName,'_Start',...
