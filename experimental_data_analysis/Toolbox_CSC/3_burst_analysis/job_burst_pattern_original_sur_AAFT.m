function job_burst_pattern_original_sur_AAFT(arrayID)

cd ..
addpath(genpath([pwd,'/Toolbox_CSC']))
addpath(genpath([pwd,'/ToolOthers/ToolNeuroPatt']))
addpath(genpath([pwd,'/ToolOthers/surrogates']))
%%
% arrayID = 8 ;
...
for nSur = 1:totalSur
    for iChannel = setdiff(1:100,badChannels)
        rng('shuffle')
        [Xs] = generate_AAFT (sigOriTemp(iChannel,:));  
        surSigTemp(iChannel,:) = Xs ;
    end
    ...
    saveFileName = ['FullBurst95%SurAAFT',dataFileName,'_Start',...
