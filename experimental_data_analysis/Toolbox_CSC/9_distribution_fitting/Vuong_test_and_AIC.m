function [R,reject_decision,p,aic,aic_power_law] = Vuong_test_and_AIC(x)
...
% fit power law
[alpha, xmin, L_log_power_law] = plfit(x);
...
aic_power_law = aicbic(L_log_power_law,2);
% fit other distribution and get ratio of log-likelihoods and p-value from
% Vuong test
distname = {'Lognormal','Normal','Poisson','Exponential','Gamma'};
 
for ii = 1:length(distname)
    [R(ii),reject_decision(ii),p(ii),aic(ii)] = get_log_likelihoods_ratio_and_AIC(x(x>=xmin),L_log_power_law,char(distname{ii}));
end
end
