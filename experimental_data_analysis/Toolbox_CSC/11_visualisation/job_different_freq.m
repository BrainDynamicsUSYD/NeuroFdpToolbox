function job_different_freq(arrayID)
...
if makeMovie
    ...
    for iTime = 1:1000
        ...
        % delta
        subplot(2,3,1)
        imagesc(squeeze(sigIn(1, :,:,iTime+timeStart)))
        set(gca,'YDir','normal')
        title(['Delta band at ', ...
        ...
        subplot(2,3,2)
        imagesc(squeeze(sigIn(2, :,:,iTime+timeStart)))
        set(gca,'YDir','normal')
        title(['Theta band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
        ...
        subplot(2,3,3)
        imagesc(squeeze(sigIn(3, :,:,iTime+timeStart)))
        set(gca,'YDir','normal')
        title(['Alpha band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
        ...
        subplot(2,3,4)
        imagesc(abs(squeeze(sigIn(4, :,:,iTime+timeStart))))
        set(gca,'YDir','normal')
        title(['Beta band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
        ...
        subplot(2,3,5)
        imagesc(squeeze(sigIn(5,:,:,iTime+timeStart)))
        set(gca,'YDir','normal')
        title(['Gamma band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
...
if makeMovieSmooth
    ...
    for iTime = 1:1000
        ...
        subplot(2,3,6)
        imagesc(abs(sigOriSmooth( :,:,sigOriStart+iTime+timeStart)).^2)
        set(gca,'YDir','normal')
        title(['Raw signal at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
            caxis([0 max(sigOriSmooth(:).^2/8 )])
        else
            colorMapSpec = pmkmp_new;
            sigLims = [-pi pi];
            colormap(gca, colorMapSpec)
