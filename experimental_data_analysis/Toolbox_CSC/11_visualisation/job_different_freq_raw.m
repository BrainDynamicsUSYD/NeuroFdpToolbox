function job_different_freq_raw(arrayID)
...
if makeMovie
    vidTitle = [pwd,'/Project2/Results/moviesRaw/',dataFileName,'_',...
    ...
    for iTime = 1:10000
        ...
        % delta
        subplot(2,3,1)
        imagesc(squeeze(sigIn(2, :,:,timeSlot)))
        set(gca,'YDir','normal')
        title(['Delta band at ', ...
        ...
        subplot(2,3,2)
        imagesc(squeeze(sigIn(3, :,:,timeSlot)))
        set(gca,'YDir','normal')
        title(['Theta band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
...
%             caxis([0 0.5*max(tempSig(:))])
        ...
        subplot(2,3,3)
        imagesc(squeeze(sigIn(4, :,:,timeSlot)))
        set(gca,'YDir','normal')
        title(['Alpha band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
...
%             caxis([0 0.5*max(tempSig(:))])
        ...
        subplot(2,3,4)
        imagesc(abs(squeeze(sigIn(5, :,:,timeSlot))))
        set(gca,'YDir','normal')
        title(['Beta band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
...
%             caxis([0 0.5*max(tempSig(:))])
        ...
        subplot(2,3,5)
        imagesc(squeeze(sigIn(6,:,:,timeSlot)))
        set(gca,'YDir','normal')
        title(['Gamma band at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
...
%             caxis([0 0.5*max(tempSig(:))])
        ...
        imagesc(squeeze(sigIn(1,:,:,timeSlot)))
        set(gca,'YDir','normal')
        title(['Raw signal at ', ...
            int2str(timeSlot/fsTemporal*1000),'ms'])
        if plotAmp
