function main_SWR_reference_gaus_yl(varargin)
% Do it!!!
% Find it!!!
% Hunt it down!!!
...
for P0_init = 0.08*ones(1,repeats)
    ...
    Type_mat(end, :) = 2;
    in_out_r = [0.13 ]; % in-out degree correlation coefficient
    
    % parameter
    for decay_GABA = 6 % [3:9] % [7:9]
        for decay_AMPA = 5 % [1:5]
            for SpikeFreqAapt = [ 1]
                                                ...
                                                for g_EI = [ 13.5 ]*10^-3 %11 12
                                                    for g_IE = [5 ]*10^-3
                                                        for g_II = [25]*10^-3
                                                            ...
                                                            for rate_ext_I = 1 % [0.8:0.1:1.2] % 1
                                                                for rate_ext_E = 0.85 % [0.75:0.1:1.15] % 0.85
                                                                    for  tau_c_EE = 8 % [7 8 9]%
                                                                        for tau_c_IE = 10 % [9 10 11]%
                                                                            for tau_c_I = 20 % [19 20 21]%
                                                                                loop_num = loop_num + 1;
                                                                                ...
                                                                                'tau_decay_GABA', decay_GABA,'tau_decay_AMPA',decay_AMPA); % SWR:3;5
                                                                                % write pop para
                                                                                ...
                                                                                writeRunawayKillerHDF5(FID, 2, min_ms, runaway_Hz, Hz_ms);
                                                                                ...
                                                                                writePopStatsRecordHDF5(FID, 2); % in order to record I neuron data
