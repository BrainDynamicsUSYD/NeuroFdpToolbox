function [MSD,tau] = get_MSD_PBC(Trajectory)

% measures the mean squared displacement (MSD) from a trajectory
% considering periodic boundary condition.
% l is the length of the grid. m = [exp(i*LFP_centre_x(1)),exp(i*LFP_centre_x(1))]; sqrt(sum((angle(m./n)/(2*pi)*63).^2))
...
D = zeros(T, T);
PCT = Trajectory(:,1:end-1);
PCT = PCT/600*2*pi; % normalization to one compplete phase range
% PCT = PCT/31*pi;

PCT = exp(1i*PCT);
D( I + T*(j-1) ) = sum((angle(PCT(I,:)./PCT(j,:))/(2*pi)*600).^2,2); % the putput is transform to actual spatial area
