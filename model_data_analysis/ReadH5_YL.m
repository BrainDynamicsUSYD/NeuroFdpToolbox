function [ OutData ] = ReadH5_YL( files )
...
if ~isempty(files)
    ...
    for id_out = 1:length(files)
        ...
        OutData{id_out}.syn_sample.u = transpose(try_h5read(files{id_out},'/syn_result_2/sample_u'));
        OutData{id_out}.syn_sample.x = transpose(try_h5read(files{id_out},'/syn_result_2/sample_x'));
%         disp('test 1')
%         OutData{id_out}.syn_sample.nsp = transpose(try_h5read(files{id_out},'/syn_result_2/sample_nsp'));
%         LP = 100;
%         for i = 1:2
...
%                 I_exc = mean(I_AMPA+I_ext);
%                 save(samp_file_mat, 'I_exc','-v7.3');
                save(samp_file_mat, 'I_AMPA', 'I_GABA','I_K','I_ext' ,'V', 'I_leak','-v7.3');
                clear 'I_AMPA'  'I_GABA' 'I_K' 'I_ext' 'V''I_leak' 'I_exc';
                fprintf('done\n');
            end
        end
