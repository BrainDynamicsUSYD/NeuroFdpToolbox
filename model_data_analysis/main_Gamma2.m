function main_Gamma2(varargin)
% adjust from function main_SWR_reference_gaus_balance.m
dt = 0.1;
sec = round(10^3/dt); % 1*(10^3/dt) = 1 sec
% step_tot = 100*sec; % use 120 second!
...
for P0_init = 0.08*ones(1,repeats)
    for step_tot = [40]*sec
        for Kw = 1.2
            for LP = [200]
                for pr = [0.9]
                    NumP = 3;IndC = [961 992 2930]; % [513 2497]; % [961 992 2930];
                    P_mat = [P0_init 0.1;
                        0.1  0.2]*2;
                    ...
                    I_ext_mean = zeros(1,N_e); % defined for each neuron (nA)
                    meanTWO = zeros(1,N_e);
                    meanTHREE = zeros(1,N_e);
                    I_ext_std = zeros(1,N_e);
                    [Lattice,~] = lattice_nD(2, hw);
                    mean_TV = zeros(1,step_tot);
                    mean_TVTWO = zeros(1,step_tot);
                    mean_TVTHREE = zeros(1,step_tot);
                    post_dist = lattice_nD_find_dist(Lattice,hw,IndC(1));
                    [~,IndP] = sort(post_dist);
                    I_ext_mean(IndP(1:LP)) = 2;
                    %                 I_ext_std(IndP(1:LP)) = 0.2;
                    mean_TV(10e4:10.35e4) = 1;
                    sample_neurons = IndP(1:LP)';
                    post_dist = lattice_nD_find_dist(Lattice,hw,IndC(2));
                    [~,IndP] = sort(post_dist);
                    meanTWO(IndP(1:LP)) = 2;
                    mean_TVTWO(20e4:20.35e4) = 1;
                    sample_neurons = [sample_neurons IndP(1:LP)'];
                    post_dist = lattice_nD_find_dist(Lattice,hw,IndC(3));
                    [~,IndP] = sort(post_dist);
                    meanTHREE(IndP(1:LP)) = 2;
                    mean_TVTHREE(30e4:30.35e4) = 1;
                    sample_neurons = [sample_neurons IndP(1:LP)'];
                    std_TV = zeros(1,step_tot);
                    % parameter
                    for decay_GABA = [6] % 6
                        for decay_AMPA = [5] % 5
                            for SpikeFreqAapt = [1]
                                for tau_K = [80] % ms 80
                                    dg_K = 0.01; % [0.005 0.015]; % uS 0.01
                                    for LFP_range_sigma = [8] % 8
                                        for cn_scale_wire = [2] % 2
                                            for cn_scale_weight = [2] % 2
                                                ...
                                                N_ext = 1000;
                                                g_ext = 2*10^-3;
                                                [ fit_g_2_EPSP_2, ~ ] = g_EPSP_conversion_SP( );
                                                for deg_hybrid = [0.4]
                                                                                ...
                                                                                for rate_ext_E = [0.85] % 0.85
                                                                                    for  tau_c_EE = [8] % the three are exponential distance dependent time constants
                                                                                        ...
                                                                                        for tau_c_I = [20]
                                                                                            ...
                                                                                            % I_ext_mean: mean for every neuron in a vector
                                                                                            % mean_TV: time-vary variable for mean for each time step
                                                                                            writeExtCurrentTimeVariantSettingsHDF5_YL(FID, 1, I_ext_mean, I_ext_std, mean_TV, std_TV, meanTWO,mean_TVTWO,meanTHREE,mean_TVTHREE);
                                                                                            ...
                                                                                            mu_p = mu_p + s_p^2;
                                                                                            g_pool_generator_hld = @(N)g_pool_generator_SP(N, mu_p, s_p);
                                                                                            ...
                                                                                            %                                                                                         K_ee_mean0 = mean(K_ee);
                                                                                            %                                                                                             [IndC,K_ee] = WorkingMemoryLocalPopulation(NumP,hw,Lattice_E,I_ee,J_ee,K_ee,Kw,LP,pr);
                                                                                            K_ee_mean = mean(K_ee);
                                                                                            %                                                                                         LPmK = K_ee_mean/K_ee_mean0;
                                                                                            ...
                                                                                            K = zeros(size(J));
                                                                                            for i_E = 1:N(1)
                                                                                                mu_K_tmp = EE_input(i_E)/sum(J==i_E)*(g_EI/g_mu)*g_balance; % *LPmK;
                                                                                            ...
                                                                                            save([sprintf('%04g-', loop_num), datestr(now,'yyyymmddHHMM-SSFFF'),...
                                                                                                ...
                                                                                                'K_ee_mean', 'LFP_centre_x', 'LFP_centre_y','IndC');
